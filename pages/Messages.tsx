...
import { fetchConversations, fetchMessages, sendMessage as sendMessageApi } from '../utils/api';
...
export const Messages: React.FC = () => {
  ...
  // Load conversations from API, fallback to localStorage
  useEffect(() => {
        ...
        try {
          ...
          if (res.success && res.data && res.data.length > 0) {
            const mapped: Conversation[] = res.data.map((c: any) => ({
              id: c.id,
              name: c.customer ? `${c.customer.firstName} ${c.customer.lastName}` : 'Customer',
              avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(c.customer?.firstName || 'C')}&background=random`,
              ...
              messages: (c.messages || []).map((m: any) => ({
                id: m.id,
                sender: m.senderId === c.merchantId ? 'me' : 'them',
                text: m.body,
  ...
  const handleSend = async () => {
    ...
    try {
        await sendMessageApi(activeChatId, userMessageText);
    } catch (error) {
        console.error("Error sending message:", error);
        // Keep the optimistic update - message is visible locally
    }
  };
               ...
               >
                 ...
                 <div className="flex-1 min-w-0">
                   <div className="flex justify-between items-baseline mb-1">
                     <h3 className={`font-bold text-sm truncate ${activeChatId === chat.id ? 'text-gray-900 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                       {chat.name}
                     </h3>
